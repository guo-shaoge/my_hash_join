...
int build_hash_table(const std::vector<std::pair<int, int>>& rows,
                     std::unordered_multimap<int, std::pair<int, int>, decltype(g_hasher), decltype(g_key_equal)>& hash_table);
void do_hash_join_per_thread(const std::unordered_multimap<int, std::pair<int, int>, decltype(g_hasher), decltype(g_key_equal)>& hash_table,
