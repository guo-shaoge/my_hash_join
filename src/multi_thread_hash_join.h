...
auto g_key_equal = [](int v1, int v2) {
  // 增加比较操作消耗，可以更明显的看出来不同ndv对join的性能影响
...
using OtherCondFunc = std::function<bool(const std::pair<int, int>&,
                                         const std::pair<int, int>&)>;
using MyHashMap = std::unordered_multimap<int, std::pair<int, int>,
                                          decltype(g_hasher), decltype(g_key_equal)>;

int scan_file(const std::string& fn,
...
void do_hash_join_per_thread(const MyHashMap* const hash_table,
                             const std::vector<std::pair<int, int>>* const probe_rows,
